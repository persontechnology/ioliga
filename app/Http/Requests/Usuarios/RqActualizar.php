...
class RqActualizar extends FormRequest
{
    ...
    public function rules()
    {
        ...
        switch ($tipoIdentificacion) {
            case 'Cédula':
                    $rqIdentificacion='not_in:0000000000|ecuador:ci|unique:users,identificacion,'.$this->input('usuario');
                break;
            case 'RUC de persona natural':
                $rqIdentificacion='ecuador:ruc|unique:users,identificacion,'.$this->input('usuario');
                break;
            case 'RUC de sociedad privada':
                $rqIdentificacion='ecuador:ruc_spriv|unique:users,identificacion,'.$this->input('usuario');
                break;
            case 'RUC de sociedad pública':
                $rqIdentificacion='ecuador:ruc_spub|unique:users,identificacion,'.$this->input('usuario');
                break;
            case 'Pasaporte':
                $rqIdentificacion='required|unique:users,identificacion,'.$this->input('usuario');
                break;
